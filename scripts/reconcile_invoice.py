...
def reconcile_invoice(invoice_number, reconciled_by="System"):
    ...
    try:
        ...
        invoices_cur.execute("""
            SELECT id, ingredient_code, ingredient_name, quantity_received,
        ...
        for line_item in line_items:
            line_id, ing_code, ing_name, qty, uom, unit_price, lot_num, exp_date = line_item
            ...
            if not ingredient:
                print(f"  WARNING: Ingredient {ing_code} ({ing_name}) not found in inventory!")
                print(f"           Skipping this line item.")
                continue

            ing_id, inv_name, current_qty, inv_uom = ingredient
            ...
            inventory_cur.execute("""
                ...
                VALUES (?, 'PURCHASE', ?, ?, ?)
            """, (ing_id, qty, unit_price, f"Invoice {invoice_number} - {supplier}"))
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage:")
        print("  python3 reconcile_invoice.py <invoice_number> [reconciled_by]")
        print("  python3 reconcile_invoice.py list")
        print("\nExamples:")
        print("  python3 reconcile_invoice.py INV-2024-001")
        print("  python3 reconcile_invoice.py INV-2024-001 'John Smith'")
        print("  python3 reconcile_invoice.py list")
        sys.exit(1)
