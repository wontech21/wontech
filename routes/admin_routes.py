...
def create_organization():
    ...
    required_fields = ['organization_name', 'slug', 'owner_name', 'owner_email']
    ...
    try:
        ...
        cursor.execute("SELECT id FROM organizations WHERE slug = ?", (data['slug'],))
        if cursor.fetchone():
            return jsonify({'error': 'Slug already exists'}), 400

        # Check if owner email is unique
        cursor.execute("SELECT id FROM organizations WHERE owner_email = ?", (data['owner_email'],))
        if cursor.fetchone():
        ...
        cursor.execute("""
            ...
            (organization_name, slug, owner_name, owner_email, phone, address, city, state, zip_code,
             plan_type, monthly_price, max_employees, features)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            ...
            data.get('monthly_price', 99.00),
            data.get('max_employees', 50),
            json.dumps(data.get('features', ['barcode_scanning', 'invoicing']))
        ))
